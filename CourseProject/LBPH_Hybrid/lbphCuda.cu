...
__global__ void elbp_kernel(const uint8_t* src, int* dst,
    ...
    for (int n = 0; n < neighbors; n++) {
        ...
        float x = static_cast<float>(radius * cos(2.0 * MY_PI * n / static_cast<float>(neighbors)));
        float y = static_cast<float>(-radius * sin(2.0 * MY_PI * n / static_cast<float>(neighbors)));
