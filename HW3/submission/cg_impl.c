               ...
               double *rnorm)
{
    ...
    //---------------------------------------------------------------------
//    #pragma omp parallel for reduction(+:rho) // No obvious benfit
	...
	#pragma omp parallel for
        ...
        //---------------------------------------------------------------------
        d = 0.0;
//	#pragma omp parallel for reduction(+:d) // No obvious benefit
        for (int j = 0; j < lastcol - firstcol + 1; j++)
        {
            d = d + p[j] * q[j];
        }
...
/*    for (int j = 0; j < lastcol - firstcol + 1; j++)
    ...
    #pragma omp parallel sections
    {
        #pragma omp section
	{
            for (int j = 0; j < lastcol - firstcol + 1; j++) norm_temp1 = norm_temp1 + x[j] * z[j];
	}
        #pragma omp section
	{
     	    for (int j = 0; j < lastcol - firstcol + 1; j++) norm_temp2 = norm_temp2 + z[j] * z[j];
	}
    }
