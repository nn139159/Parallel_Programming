...
int main(int argc, char **argv)
{
	...
	// Size of the input and output images on the host
        const unsigned long data_size = image_height * image_width * sizeof(float);
        cuda_image = (float *)malloc(data_size);

        double record_cuda[10] = {0};
        for (int i = 0; i < 10; ++i)
        {
            memset(cuda_image, 0, data_size);
            start_time = current_seconds();

            // 呼叫 CUDA host function
            host_fe_cuda(filter_width, filter, image_height, image_width,
                         input_image, cuda_image);

            end_time = current_seconds();
            record_cuda[i] = end_time - start_time;
        }

        qsort(record_cuda, 10, sizeof(double), compare);
        for (int i = 3; i < 7; ++i)
            min_cuda += record_cuda[i];
        min_cuda /= 4;

        printf("\n[conv cuda]:\t\t[%.3f] ms\n\n", min_cuda * 1000);

        // Write the output image to file
        store_image(cuda_image, "output_cuda.bmp", image_height, image_width, input_file);
        free(input_image);
        free(filter);
    }
    ...
    for (i = 0; i < image_height; i++)
    {
        for (j = 0; j < image_width; j++)
        {
            ...
            if (fabsf(cuda_image[(i * image_width) + j] - ref_image[(i * image_width) + j]) > 10)
            {
                diff_counter_cuda += 1;
            }
        }
    ...
    printf("Diff ratio: %f\n", diff_ratio);
    float diff_ratio_cuda = (float)diff_counter_cuda / (image_height * image_width);
    printf("Diff ratio (CUDA):   %f\n", diff_ratio_cuda);

    if (diff_ratio > 0.1 || diff_ratio_cuda > 0.1)
    {
    ...
    printf("\n\033[32mCUDA Speedup: %.2fx over serial\033[0m\n", min_serial / min_cuda);
    return 0;
}
