...
int main(int argc, char **argv)
{
    ...
    {
        // TODO: MPI workers
	// MPI workers send their counts to rank 0
    ...
    else if (world_rank == 0)
    {
        ...
        long long int *receive_counts = (long long int *)malloc((world_size - 1) * sizeof(long long int));
        MPI_Request *requests = (MPI_Request *)malloc((world_size - 1) * sizeof(MPI_Request));

        for (int i = 1; i < world_size; i++)
        {
            MPI_Irecv(&receive_counts[i - 1], 1, MPI_LONG_LONG, i, 0, MPI_COMM_WORLD, &requests[i - 1]);
        }
